...
try {
    ...
    try {
        ...
        Write-Information "Querying FullAccess permissions to mailbox [$($exchangeQuerySplatParams.Identity)]"
        $mailboxPermissions = Get-MailboxPermission @exchangeQuerySplatParams | Where-Object { ($_.accessRights -like "*fullaccess*") -and -not($_.Deny -eq $true) } -ErrorAction Stop
        ...
        throw "Error querying FullAccess permissions to mailbox [$($exchangeQuerySplatParams.Identity)]. Error Message: $($errorMessage.AuditErrorMessage)"
    }
}
catch {
    $ex = $PSItem
    ...
    Write-Error "Error querying FullAccess permissions to mailbox [$($mailboxGuid)]. Error Message: $($errorMessage.AuditErrorMessage)"
}